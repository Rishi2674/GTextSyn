...
from module.Ablation_model import GTextSyn, GTextSyn_GCN, GTextSyn_GAT, GTextSyn_LSTM, GTextSyn_NOATT
from utils_ import metrics, dataset, DataLoader, log, collate_merg
...
def train(traindata, testDL):
    ...
    for epoch in range(P["EPOCHES"]):
        ...
        for i, (train_ind, valid_ind) in enumerate(KF.split(X)):
            ...
            trainDL = DataLoader(_d, batch_size=P["BATCH_SIZE"], shuffle=True, collate_fn=lambda x: collate_merg(x, device))
            _d = dataset(split_validx, split_validy, device)
            validDL = DataLoader(_d, batch_size=P["TEST_BATCH"], shuffle=True, collate_fn=lambda x: collate_merg(x, device))
            ...
            test_loss, _, _, test_r2, test_pearson = test(testDL, "test")
            
            log("model", MODEL_NAME, loss_mean, valid_loss.item(), test_loss.item(), r2, pearson, f"--{epoch+1}--{i}")
...
def main(study):
    ...
    global MODEL_NAME
    MODEL_NAME = f"{study}_{MODEL}_" + MODEL_NAME

    with open(f"./data/{study}_train.pkl", 'rb') as fp:
        ds = pickle.load(fp)
    X = [i[0] for i in ds]
    Y = [i[1] for i in ds]
    ...
    log("model", MODEL_NAME, 0, 0, mse.item(), r2, pearson, "---test---")
